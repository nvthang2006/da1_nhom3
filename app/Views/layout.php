...
$scriptName = $_SERVER['SCRIPT_NAME']; // ví dụ: /DA1/index.php
$base = rtrim($scriptName, '/');       // giữ nguyên đường dẫn tới file index.php
$user = null;
if (class_exists('\App\Core\Auth')) {
    $user = \App\Core\Auth::user();
}

// $viewFile should be set by controller and file exists check done below.
// $tours may be set by controller when rendering home or tours page.
?>
<!doctype html>
<html lang="vi">
    ...
    <header class="site-header">
        <div class="container header-inner">
            ...
            <nav class="main-nav" aria-label="Chính">
                ...
                <a href="<?= $base ?>/tours?type=domestic">Trong nước</a>
                <a href="<?= $base ?>/tours?type=international">Quốc tế</a>
            ...
            <div style="display:flex;gap:10px;align-items:center">
                <?php if ($user): ?>
                    <?php if ($user['role'] === 'admin'): ?>
                        <a href="<?= $base ?>/admin/tours" class="cta">Quản trị</a>
                    <?php elseif ($user['role'] === 'hdv'): ?>
                        <a href="<?= $base ?>/hdv" class="cta">HDV</a>
                    <?php endif; ?>
                    <a href="<?= $base ?>/logout" style="text-decoration:none;color:var(--muted)"><?= htmlspecialchars($user['full_name'] ?? $user['email']) ?> — Đăng xuất</a>
                <?php else: ?>
                    <a href="<?= $base ?>/login" class="cta">Đăng nhập</a>
                <?php endif; ?>
